var Type_Mantenences = {
    process_form : function(){
        
        let form = $('#form').serializeArray();
        let url = $('#form').attr('action');
        let message_error = Type_Mantenences.validate_form(form);
        console.log(form);
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.status == 200){
                    ...
                    window.location.href = window.location.origin + '/mi-escuela/configurarciones/tipos-mantenimeinto';
                    return false;
                }
